...
export class ScenarioComparisonMap {
  ...
  _updateCountiesLayer() {
    ...
    return this._createReferenceLayer(this.options.countiesLayerURL, {
      ...
      this._mapInitPromise.then(() => {
        ...
        // this.view.on("click",
        //     /**
        //      * @param event {{mapPoint}}
        //      */
        //     (event) => {
        //       // noinspection JSUnresolvedFunction
        //       this.countiesLayer.queryFeatures({
        ...
        //           $.get('../resources/data/fips_codes.json', (jsondata) => {
  ...
  async _getAreaStatsPopupContent(feature) {
    ...
    const url = `/local-climate-charts/?fips=${area_id}&lat=${get(feature, ['graphic', 'geometry', 'centroid', 'latitude'])}&lon=${get(feature, ['graphic', 'geometry', 'centroid', 'longitude'])}&city=${area_name}${area_type === 'county' ? `&county=${get(feature, ['graphic', 'attributes', 'NAME'])} County` : ''}&nav=local-climate-charts`
